...
public static boolean debMode= true;
    ...
    public static void main(String[] args) throws Exception {
		...
		System.out.println("");
		System.out.println("                         version diciembre-2015                          ");
		System.out.println("");
		System.out.println("");
		System.out.println("----------------------------------------------------------------------");
		System.out.println("");
...
//			}
			...
			pOutFName=	datasimulation.pOutFName[0];
...
//			PrintWriter pw 		= new PrintWriter( new FileWriter(unitfileroot+"SolutionUC.csv"));
			...
			for(int k=0;k<id_line.length;k++){
				...
				((LineUC)(vlineuc.elementAt(k))).InitUC(((DataUCLine)(vline.elementAt(k))).ID,((DataUCLine)(vline.elementAt(k))).Nombre,((DataUCLine)(vline.elementAt(k))).BusIni,((DataUCLine)(vline.elementAt(k))).BusFin, ((DataUCLine)(vline.elementAt(k))).Resistencia, ((DataUCLine)(vline.elementAt(k))).Reactancia, ((DataUCLine)(vline.elementAt(k))).Largo, ((DataUCLine)(vline.elementAt(k))).Voltaje, ((DataUCLine)(vline.elementAt(k))).Fmin, ((DataUCLine)(vline.elementAt(k))).Fmax, ((DataUCLine)(vline.elementAt(k))).Propietario, ((DataUCLine)(vline.elementAt(k))).matriz_alpha);
			...
			System.out.println("");
			problem.formulate(cplex, vt, vh, vsoluc, veol, veoluc, vbat , vbatuc, vpump, vpumpuc, vres, vresuc, vload, vloaduc, vcsp, vcspuc, vlineuc, vbusuc, restric_demanda, id_tgenerator,  id_hgenerator, id_reserv, id_hdb_ser, id_hdb_paspur, id_hdb_irr, id_hdb_affl, id_busbar, id_line, id_load, id_solar, id_eolic, id_battery, id_pump, id_csp, H, T, UnitT, demanda, Spinning, Spinning2, CPF, ERNCLimit, Cpvfic, generacion_eolic, generacion_solar, generacion_renovable, sum_pmin, fes, fcon,bus,bandera_perdida);
			System.out.println("");
		...
		try{	
			...
			//Demanda real
			print_solution(pw,demanda[0],"Demanda real",H, T);
			//Demanda
...
//**				}
			...
			for(int i=0;i<id_line.length;i++){
				type_aux="Linea";
				//flujo linea
				print_solution_line(pw15,(LineUC)(vlineuc.elementAt(i)), Fsol,type_aux,"Flujo", "Flujo", H, T, i);
			}
